...
$groups = Import-Csv $csv #Import list of groups from csv file
$manager = Get-ADUser -Filter "Name -eq $owner" | Select samaccountname #Collect samaccountname of Owner
foreach ($group in $groups) {
New-ADGroup -Name $group.name -Path $OU -Description “New Groups Created in Bulk” -GroupCategory Security -GroupScope Universal -Managedby $manager #Create a group for each item in csv
}